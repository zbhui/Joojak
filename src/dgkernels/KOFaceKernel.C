...
KOFaceKernel::KOFaceKernel(const std::string & name, InputParameters parameters):
...
Real KOFaceKernel::computeQpResidual(Moose::DGResidualType type)
{
	Real CIP = computeCIP();
...
Real KOFaceKernel::computeQpJacobian(Moose::DGJacobianType type)
{
	Real r = 0;
	Real CIP = computeCIP();
	int p(_eq), q(_eq);
...
Real KOFaceKernel::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
	Real r = 0;
	Real CIP = computeCIP();
	int p(_eq), q(jvar);
