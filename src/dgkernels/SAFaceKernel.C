...
#include "SAFaceKernel.h"

template<>
InputParameters validParams<SAFaceKernel>()
{
	  InputParameters params = validParams<DGKernel>();
	  params += validParams<SABase>();
	  return params;
}
SAFaceKernel::SAFaceKernel(const std::string & name, InputParameters parameters):
...
Real SAFaceKernel::computeQpResidual(Moose::DGResidualType type)
{
	Real CIP = computeCIP();
...
Real SAFaceKernel::computeQpJacobian(Moose::DGJacobianType type)
{
	Real r = 0;
	Real CIP = computeCIP();
	int p(_eq), q(_eq);
...
Real SAFaceKernel::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
	Real r = 0;
	Real CIP = computeCIP();
	int p(_eq), q(jvar);
