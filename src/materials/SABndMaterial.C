...
SABndMaterial::SABndMaterial(const std::string & name, InputParameters parameters):
...
void SABndMaterial::computeQpProperties()
{
	if(!_bnd)
	{
...
void SABndMaterial::computeQpLeftValue(Real* ul, RealGradient *dul)
{
...
void SABndMaterial::computeQpRightValue(Real *ur, RealGradient *dur, Real *ul, RealGradient *dul)
{
...
void SABndMaterial::fluxTerm(Real *flux, Real* ul, Real* ur, RealGradient *dul, RealGradient *dur)
{
...
void SABndMaterial::penaltyTerm(RealVectorValue* penalty, RealVectorValue* penalty_neighbor, Real* ul, Real* ur)
{
	RealGradient duh[10];
	for (int eq = 0; eq < _n_equations; ++eq)
...
void SABndMaterial::isothermalWall(Real *ur, RealGradient *dur, Real *ul, RealGradient *dul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		dur[eq] = dul[eq];
...
void SABndMaterial::adiabaticWall(Real* ur, RealGradient* dur, Real* ul, RealGradient* dul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		dur[eq] = dul[eq];
...
void SABndMaterial::farFieldRiemann(Real *ur, RealGradient *dur, Real *ul, RealGradient *dul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		dur[eq] = dul[eq];
...
void SABndMaterial::farField(Real *ur, RealGradient *dur, Real *ul, RealGradient *dul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		dur[eq] = dul[eq];
...
void SABndMaterial::symmetric(Real *ur, RealGradient *dur, Real *ul, RealGradient *dul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		dur[eq] = dul[eq];
...
void SABndMaterial::pressureOut(Real *ur, RealGradient *dur, Real *ul, RealGradient *dul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		dur[eq] = dul[eq];
...
void SABndMaterial::resizeQpProperty()
{
...
void SABndMaterial::viscousTerm(RealVectorValue* viscous_term, Real* uh, RealGradient* duh)
{
	if(_bc_type == "adiabatic_wall")
	{
		Real rho = uh[0];
	...
	{
		SABase::viscousTerm(viscous_term, uh, duh);
	}
}
