...
KOCellMaterial::KOCellMaterial(const std::string & name, InputParameters parameters):
...
void KOCellMaterial::computeQpProperties()
{
	...
	sourceTerm(&_source_term[_qp][0], uh, duh);
	...
	for (int q = 0; q < _n_equations; ++q)
	{
		...
		sourceTerm(source_term_new, uh, duh);
...
void KOCellMaterial::computeQpValue(Real* uh, RealGradient *duh)
{
	for (int eq = 0; eq < _n_equations; ++eq)
	{
		uh[eq] = (*_uh[eq])[_qp];
...
void KOCellMaterial::resizeQpProperty()
{
...
void KOCellMaterial::fluxTerm(RealVectorValue* flux_term, Real* uh, RealGradient* duh)
{
