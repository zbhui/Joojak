...
template<>
InputParameters validParams<KOFaceMaterial>()
{
...
KOFaceMaterial::KOFaceMaterial(const std::string & name, InputParameters parameters):
...
void KOFaceMaterial::computeQpProperties()
{
	if(_bnd && _neighbor)
	{
		resizeQpProperty();
...
void KOFaceMaterial::computeQpLeftValue(Real* ul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		ul[eq] = (*_ul[eq])[_qp];
}

void KOFaceMaterial::computeQpRightValue(Real* ur)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		ur[eq] = (*_ur[eq])[_qp];
}

void KOFaceMaterial::computeQpLeftGradValue(RealGradient *ul)
{
	for (int eq = 0; eq < _n_equations; ++eq)
		ul[eq] = (*_grad_ul[eq])[_qp];
}

void KOFaceMaterial::computeQpRightGradValue(RealGradient *ur)
{
	for (int eq = 0; eq < _n_equations; ++eq)
...
void KOFaceMaterial::penaltyTerm(RealVectorValue* penalty, RealVectorValue* penalty_neighbor, Real* ul, Real* ur)
{
	RealGradient duh[10];
	for (int eq = 0; eq < _n_equations; ++eq)
...
void KOFaceMaterial::resizeQpProperty()
{
